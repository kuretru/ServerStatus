...
int CMain::HandleMessage(int ClientNetID, char *pMessage)
{
	...
	if(str_comp_num(pMessage, "update", sizeof("update")-1) == 0)
	{
		...
		if(rStart["real_used"].type)
			pClient->m_Stats.m_RealUsed = rStart["real_used"].u.integer;
